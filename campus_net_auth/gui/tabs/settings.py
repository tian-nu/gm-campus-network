...
class SettingsTab(Frame):
    ...
    enable_network_whitelist_check: SettingCheckbox | None
    network_name_whitelist_var: StringVar | None
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enable_network_whitelist_check: SettingCheckbox | None = None
        self.network_name_whitelist_var: StringVar | None = None
    ...
    def _create_widgets(self) -> None:
        ...
        # 不设初始 width，等 Configure 事件修正
        _ = canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw")
    ...
    def _create_network_settings(self, parent: Widget) -> None:
        ...
        # 网络名称白名单
        whitelist_group = SettingGroup(parent, title="网络名称白名单")
        whitelist_group.pack(fill=X, pady=(0, 10))

        self.enable_network_whitelist_check = SettingCheckbox(
            whitelist_group,
            text="仅允许在指定网络中登录"
        )
        self.enable_network_whitelist_check.pack(anchor=W, pady=3)

        whitelist_frame = Frame(whitelist_group)
        whitelist_frame.pack(fill=X, pady=3)
        ...
        self.network_name_whitelist_var = StringVar()
        whitelist_entry = Entry(
            whitelist_frame,
            textvariable=self.network_name_whitelist_var,
            font=("Microsoft YaHei UI", 9),
            width=40
        )
        whitelist_entry.pack(side=LEFT, padx=(10, 0), fill=X, expand=True)

        Label(
            whitelist_group,
            text="多个名称用英文逗号分隔，如: GM-living,东1-living",
    ...
    def get_config(self) -> dict[str, object]:
        ...
        return {
            ...
            # 网络名称白名单
            "enable_network_whitelist": self.enable_network_whitelist_check.get(),
            "network_name_whitelist": self.network_name_whitelist_var.get(),
        }
    ...
    def set_config(self, config: dict) -> None:
        ...
        # 网络名称白名单（去掉换行，保证单行显示）
        whitelist_raw = config.get("network_name_whitelist", "")
        whitelist_clean = whitelist_raw.replace("\n", "").replace("\r", "").replace(" ", "")
        self.enable_network_whitelist_check.set(config.get("enable_network_whitelist", True))
        self.network_name_whitelist_var.set(whitelist_clean)
